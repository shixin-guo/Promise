...
class AjaxUploader extends Component<UploadProps> {
  ...
  onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const acceptedFiles =
      ...
      [...files].filter(
        (file: File) =>
          !directory || attrAccept(file as RcFile, accept as string)
      )
    this.uploadFiles(acceptedFiles as File[])
    this.reset()
  }
  ...
  processFile = async (
    ...
    let mergedAction: string | null | undefined = null
  ...
  post({ data, origin, action, parsedFile }: ParsedFileInfo) {
    ...
    const requestOption = {
      ...
      onProgress: (e: UploadProgressEvent) => {
        ...
        parsedFile && onProgress?.(e, parsedFile)
      },
      onSuccess: (ret: any, xhr: XMLHttpRequest) => {
        const { onSuccess } = this.props
        parsedFile && onSuccess?.(ret, parsedFile, xhr)

        delete this.requestWithUID[uid]
      },
      onError: (err: UploadRequestError, ret: any) => {
        const { onError } = this.props
        parsedFile && onError?.(err, ret, parsedFile)
    ...
    onStart?.(origin)
    this.requestWithUID[uid] = request(
      requestOption as UploadRequestOption<any>
    )
  }
  ...
  abort(file?: any) {
    const { requestWithUID } = this
    if (file) {
      const uid = file.uid ? file.uid : file
      if (requestWithUID[uid] && requestWithUID[uid].abort) {
        requestWithUID[uid].abort()
      }
      delete requestWithUID[uid]
    } else {
      Object.keys(requestWithUID).forEach((uid) => {
        if (requestWithUID[uid] && requestWithUID[uid].abort) {
          requestWithUID[uid].abort()
        }
        delete requestWithUID[uid]
